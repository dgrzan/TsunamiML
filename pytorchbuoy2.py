...
class MLP(nn.Module):
    ...
    def __init__(self, layersize, resize):
        ...
        self.layers = nn.Sequential(
            ...
            nn.Conv1d(14, 5, 1, stride=1, padding=0),
            nn.ReLU(),
    ...
    """
            ...
            nn.ReLU(),
            nn.Conv1d(4, 4, 5, stride=5),
            nn.ReLU(),
            nn.Conv1d(4, 4, 3, stride=1, padding=1),
            nn.ReLU(),
            nn.Conv1d(4, 4, 5, stride=5),
            nn.ReLU(),
            nn.Conv1d(4, 8, 3, stride=1, padding=1),
            nn.ReLU(),
...
if __name__ == '__main__':
    ...
    #prepare the dataset
    with open('/home/davidgrzan/Tsunami/cascadia/machinelearning/buoyinput.txt', "r") as csvfile:
    ...
    for epoch in range(0, epochs): 
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if i % 500 == 499:
                ...
                current_loss = 0.0
        if(epoch==epochs-1):
            myloss = float(myloss/(3000-testnumber))
            print("custom loss: "+str(myloss))
            
        #finds error of validation set for that epoch
        for k in range(testnumber):
            validationoutputs = mlp(xtest[k].float())
            validationloss = loss_function(validationoutputs, ytest[k].float())
            validationlossavg += validationloss.item()
        validationlossavg = float(validationlossavg/testnumber)
    ...
    plt.plot(epochlossarray,"blue")
    plt.plot(validationlossarray,"red")
    plt.show()
    ...
    out_dataset = Dataset("/home/davidgrzan/Tsunami/cascadia/machinelearning/buoy8.nc", 'w', format='NETCDF4')
