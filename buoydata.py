...
if __name__ == "__main__":
    ...
    #data is organized by stacking the 14 buoys on top of each other for each entry, then continuing down the line for all 3000
    for jj in range(10,100,5):
        ...
        for i in range(3000):
            ...
            if(i==0):
                #locationarr = locations("locationsbuoys3.txt")
                #indexarr = find(locationarr, simlatitude, simlongitude)
                indexarr, amount = gridindex(spacing, altitude, simlatitude, simlongitude)
                outputar = np.zeros((int(3000*amount),50)) #every 16 seconds for all 50 entries (13 min)
                print(amount)
            buoyheights = heights(indexarr, upliftdata, 50) #the last number is how many time steps we want to record, 50 is max
            for j in range(amount):
                outputar[i*amount+j] = buoyheights[j]
        
        np.savetxt("/home/davidgrzan/Tsunami/cascadia/machinelearning/batch/input/buoyinputgrid"+str(spacing)+".txt", outputar, delimiter=",", fmt="%.3f")
        
        if(True):
            for i in range(len(indexarr)):
                altitude[0,int(indexarr[i][1]),int(indexarr[i][0])] = 10000000
                
            out_dataset = Dataset("/home/davidgrzan/Tsunami/cascadia/machinelearning/batch/input/buoyinputgrid"+str(spacing)+".nc", 'w', format='NETCDF4')
